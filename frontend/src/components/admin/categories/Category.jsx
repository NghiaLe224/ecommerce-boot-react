...
import { deleteCategoryDashboard } from "../../../store/category-slice";
...
const Category = () => {
  ...
  const { categories, pagination, loading: categoryLoader, error: errorMessage } = useSelector((state) => state.category);
  ...
  dispatch(deleteCategoryDashboard(selectedCategory?.id))
    .unwrap()
    .then(() => {
    ...
    .catch((err) => {
      toast.error(
          typeof err === "string" ? err : err?.message || "Delete category failed"
        )
    });
};
