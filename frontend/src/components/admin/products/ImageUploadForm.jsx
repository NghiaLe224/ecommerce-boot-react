...
import { updateProductImageFromDashboard } from "../../../store/product-slice";
...
const ImageUploadForm = ({ setOpen, product }) => {
  ...
  const addNewImageHandler = async (event) => {
    ...
    try {
      ...
      toast.success(res?.message || "Product image updated successfully");
      setOpen(false);
    } catch (error) {
      toast.error(error || "Failed to update product image");
    } finally {
      setLoader(false);
    }
  };
